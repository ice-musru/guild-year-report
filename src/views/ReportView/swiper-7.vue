...
const anchor_live_time_rank = computed(() => {
  const arr = store.getters.guildData.anchor_live_time_rank || [];
  if (arr.length < 5) {
...
<template>
  <div class="swiper-7 h-full relative">
    <div class="swiper-frame relative">
      <div class="swiper-frame-bg relative">
        <!--    蛋糕    -->
        <div class="top-cake absolute left-0 -top-20px ani z-1" swiper-animate-effect="animate__fadeInDown" swiper-animate-duration="0.6s" swiper-animate-delay="0s"></div>
        <!--    信封    -->
        <div class="bottom-letter absolute right-0 -bottom-50px ani z-1" swiper-animate-effect="animate__fadeInUp" swiper-animate-duration="0.8s" swiper-animate-delay="0s"></div>
        <!--   标题   -->
        ...
        <div class="rank-list absolute top-200px left-60px">
          <div
            ...
            :class="[`item-${index + 1}`]"
            v-for="(item, index) in anchor_live_time_rank"
            :key="index"
          ...
          >
            ...
            <div class="item-word inline-flex justify-between items-center h-88px">
              ...
              <span class="item-word__num" v-if="item.rank !== -1">{{ item.score }}小时</span>
            </div>
          </div>
        </div>
        <!--   文字描述     -->
        <div class="rank-word absolute bottom-120px left-80px">
          <p class="text-28px mb-10px ani" swiper-animate-effect="animate__fadeInUp" swiper-animate-duration="0.8s" swiper-animate-delay="0.8s">越努力越幸运</p>
          <p class="text-28px ani" swiper-animate-effect="animate__fadeInUp" swiper-animate-duration="0.8s" swiper-animate-delay="0.9s">这份付出相信观众一定会看到</p>
        </div>
      </div>
    </div>
  </div>
...
.rank-title-3 {
  width: 578px;
  height: 107px;
  background: url("../../assets/images/swiper-6-11/swiper-6-11-title-1.png") no-repeat center / contain;
}
...
.top-cake {
  width: 110px;
  height: 150px;
  background: url("../../assets/images/swiper-6-11/swiper-6-11-cake.png") no-repeat center / contain;
}
.bottom-letter {
  width: 266px;
  height: 210px;
  background: url("../../assets/images/swiper-6-11/swiper-6-11-letter.png") no-repeat center / contain;
}
</style>
